...
parser.add_argument('--init_temp', type=float, default=10, help='starting temperature for temperature anealing')
parser.add_argument('--min_temp', type=float, default=0.1, help='end temperature for temperature anealing')
parser.add_argument('--fixed_temp', type=float, default=10.0, help='use if ta is False')
...
def main():
    ...
    temp_scheduler = []
    if args.ta == True:
        # temparature scheduler
        temp_start = args.init_temp
        temp_end = args.min_temp
        temp_standard = (temp_start-temp_end)/(args.epochs-1)
        for i in range(args.epochs):
            temp = temp_start-(i*temp_standard)
    ...
    model = Network(args.init_ch, 100, args.layers, criterion, args.init_temp).to(device)
