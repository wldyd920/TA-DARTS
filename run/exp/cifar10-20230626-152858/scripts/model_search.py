...
class Network(nn.Module):
    ...
    """
    def __init__(self, c, num_classes, layers, criterion, temperature, steps=4, multiplier=4, stem_multiplier=3):
        """
    ...
    def new(self, temperature):
        ...
        """
        self.temperature = temperature
        model_new = Network(self.c, self.num_classes, self.layers, self.criterion, temperature).cuda()
    ...
    def forward(self, x):
        ...
        for i, cell in enumerate(self.cells):
            if cell.reduction: 
                weights = F.softmax(self.alpha_reduce/self.temperature, dim=-1) 
            else:
                weights = F.softmax(self.alpha_normal/self.temperature, dim=-1) 
            s0, s1 = s1, cell(s0, s1, weights)

        # # Gumbel Softmax
        # for i, cell in enumerate(self.cells):
        #     if cell.reduction: 
        #         weights = F.gumbel_softmax(self.alpha_reduce, dim=-1)
        #     else:
        #         weights = F.gumbel_softmax(self.alpha_normal, dim=-1)
        #     s0, s1 = s1, cell(s0, s1, weights)
    ...
    def genotype(self):
        ...
        gene_normal = _parse(F.softmax(self.alpha_normal/self.temperature, dim=-1).data.cpu().numpy())
        gene_reduce = _parse(F.softmax(self.alpha_reduce/self.temperature, dim=-1).data.cpu().numpy())

        # # Gumbel Softmax
        # gene_normal = _parse(F.gumbel_softmax(self.alpha_normal, dim=-1).data.cpu().numpy())
        # gene_reduce = _parse(F.gumbel_softmax(self.alpha_reduce, dim=-1).data.cpu().numpy())
